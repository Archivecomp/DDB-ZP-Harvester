        ...
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                if stripped.startswith("@prefix"):
                    continue
                elif re.match(r"^<.+?> a nfdicore:Organization \.$", stripped):
                    org_statements.add(stripped)
                else:
                    cleaned_lines.append(line)
            chunk_body = "\n".join(cleaned_lines).strip()
            if chunk_body:
                all_ttl.append(chunk_body)

    # Append unique org definitions
    if org_statements:
